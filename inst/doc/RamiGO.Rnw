...
\begin{description}
\item[AmiGO vizualise:] \url{http://amigo.geneontology.org/cgi-bin/amigo/amigo?mode=visualize}
\end{description}
...
%------------------------------------------------------------ 
At first we load the \Rpackage{RamiGO} into the current workspace:
...
<<adjM2gml>>=
adjM2gml(adjMatrix(tt),relations(tt)$color,annot(tt)$fillcolor,annot(tt)$GO_ID,annot(tt)$description,"example")
@
