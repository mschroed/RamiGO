...
%------------------------------------------------------------ 
The DOT format parser uses the \Rfunction{strapply} function from the \Rpackage{gsubfn} package to extract the information from the DOT format file. Therefor the  \Rpackage{gsubfn} package has to be installed on your system, as well as the following packages:
...
The \Rpackage{RCurl} package is useful for communicating with webserver and sending GET or POST requests. RamiGO uses the \Rfunction{postForm()} function to communicate with the AmiGO webserver. The \Rpackage{png} package is used to convert the webserver response for an png request into an actual png file. The \Rpackage{igraph} package is used to build an graph object representing the tree that was parsed from an DOT format file.
...
The leaves of the tree are returned in \Robject{tt\$leaves}:
...
<<adjM2gml>>=
adjM2gml(tt$adjMatrix,tt$relations$color,tt$annot$fillcolor,tt$annot$GO_ID,tt$annot$description,"example")
@
